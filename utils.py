...
#from exceptions import IncorrectChannelTypeException
...
emojidict: emojidictionary = emojidictionary({
    ...
    'loading': '<a:loading:1181644325445193778>',
...
class FiveButtonPaginator(ButtonPaginator):
...
LOADING_EMOJI = emojidict.get('loading')
...
class SCRStatsBot(Bot):
    ...
    async def getorfetch_thread(self, threadid: int, guild: Guild) -> Thread:
        ...
        if isinstance(ch, Thread):
            return ch
        raise IncorrectChannelTypeException(f"Channel {threadid} is not a Thread")
    ...
    async def getorfetch_textchannel(self, channelid: int, guild: Guild) -> TextChannel:
        ...
        if isinstance(ch, TextChannel):
            return ch
        raise IncorrectChannelTypeException(f"Channel {channelid} is not a TextChannel")
    ...
    async def getorfetch_voicechannel(self, channelid: int, guild: Guild) -> VoiceChannel:
        ...
        if isinstance(ch, VoiceChannel):
            return ch
        raise IncorrectChannelTypeException(f"Channel {channelid} is not a VoiceChannel")
    ...
    async def getorfetch_categorychannel(self, channelid: int, guild: Guild) -> CategoryChannel:
        ...
        if isinstance(ch, CategoryChannel):
            return ch
        raise IncorrectChannelTypeException(f"Channel {channelid} is not a CategoryChannel")
    ...
    async def getorfetch_stagechannel(self, channelid: int, guild: Guild) -> StageChannel:
        ...
        if isinstance(ch, StageChannel):
            return ch
        raise IncorrectChannelTypeException(f"Channel {channelid} is not a StageChannel")
    ...
    async def getorfetch_forumchannel(self, channelid: int, guild: Guild) -> ForumChannel:
        ...
        if isinstance(ch, ForumChannel):
            return ch
        raise IncorrectChannelTypeException(f"Channel {channelid} is not a ForumChannel")
    ...
    def instance_makeembed(*args, **kwargs):
        return SCRStatsBot.makeembed(*args, **kwargs)
    
    @staticmethod
    def makeembed_bot(title: Optional[str]=None,timestamp: datetime.datetime=datetime.datetime.now(),
        ...
        return SCRStatsBot.makeembed(title=title,timestamp=timestamp,color=color,description=description,author=author,author_url=author_url,author_icon_url=author_icon_url,footer=footer,footer_icon_url=footer_icon_url,url=url,image=image,thumbnail=thumbnail)

    def instance_makeembed_bot(*args, **kwargs):
        return SCRStatsBot.makeembed_bot(*args, **kwargs)
...
def is_owner(user: Union[discord.User, discord.Member], bot: SCRStatsBot):
    assert bot.owner_ids is not None
    return user.id in bot.owner_ids

def check_is_trusted(user: Union[discord.User, discord.Member], bot: SCRStatsBot):
...
makeembed_bot = SCRStatsBot.makeembed_bot
makeembed = SCRStatsBot.makeembed
parsetime = SCRStatsBot.parsetime
dctimestamp = SCRStatsBot.dctimestamp
dchyperlink = SCRStatsBot.dchyperlink
